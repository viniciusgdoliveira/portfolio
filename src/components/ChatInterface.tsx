...
export default function ChatInterface() {
	...
	const { messages, sendMessage, isLoading, error, clearConversation, shouldWarn, tokensUsed, rateLimit } = useChat();
	...
	return (
		<div className="h-full flex flex-col w-full mobile-scroll-container">
			<div className="liquid-card p-6 mb-6 flex-shrink-0">
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center space-x-4">
						...
						{/* Token usage */}
						{tokensUsed > 0 && <div className="text-white/60 text-sm">{t("status.tokensUsed", { count: tokensUsed })}</div>}
			...
			{/* Chat container */}
			<div className="liquid-card overflow-hidden flex flex-col flex-1 min-h-0">
				{/* Messages */}
				<div
					ref={chatContainerRef}
					className="flex-1 overflow-y-auto p-6 chat-scrollbar min-h-0"
				>
					{/* Welcome message */}
					{messages.length === 0 && (
						<div className="text-center py-10">
							...
							<p className="text-white/80 mb-6 max-w-md mx-auto">{t("greeting")}</p>
					...
					{messages.map((message) => (
						...
						>
							<div className={`flex ${message.role === "user" ? "flex-row-reverse" : "flex-row"} items-end space-x-3 max-w-[85%]`}>
								...
								{/* Message bubble */}
								<div className={`chat-bubble relative px-6 py-4 rounded-[24px] ${message.role === "user" ? "chat-bubble-user text-white ml-12" : "chat-bubble-assistant text-white/95"}`}>
									{/* Message content */}
									...
									{/* Timestamp */}
									<div className={`chat-timestamp mt-3 flex items-center space-x-2 ${message.role === "user" ? "text-blue-100" : "text-white/50"}`}>
				...
				<div className="border-t border-white/10 p-4 sm:p-6 flex-shrink-0">
					...
					>
						<div className="flex-1 relative">
							...
							{/* Character counter */}
							<div className="absolute bottom-2 right-2 text-xs text-white/40">{input.length}</div>
						</div>
					...
					{rateLimit && (
						<div className="mt-3 flex items-center justify-between text-sm">
							<div className="flex items-center space-x-2 text-white/60">
								...
								<span>{rateLimit.remaining > 0 ? t("rateLimit.remaining", { remaining: rateLimit.remaining }) : t("rateLimit.limitReached")}</span>
							</div>
							{rateLimit.remaining <= 5 && rateLimit.remaining > 0 && <div className="text-yellow-400 text-xs">{t("rateLimit.approachingLimit")}</div>}
						</div>
					)}
